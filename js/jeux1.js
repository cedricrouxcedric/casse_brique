var canvas = document.getElementById("myCanvas"); // association a la balise canavas
    var ctx = canvas.getContext("2d"); // context 2d
    var ballRadius = 10; // rayon de la balle   
    var x = canvas.width/2; // position x du centre de la balle
    var y = (canvas.height-10)-10; // position y du centre de la balle
    var dx = 2; // vitesse horizontale
    var dy = 2; // vitesse verticale
    var paddleHeight = 10; // hauteur du paddle
    var paddleWidth = 70; // largeur du paddle
    var paddleX = (canvas.width-paddleWidth)/2; // position x du coin haut gauche du paddle
    var rightPressed = false;  // booleen droite
    var leftPressed = false;  // booleen gauche
    var brickRowCount = 5; // nbr de lignes de briques
    var brickColumnCount = 6; // nbr de colonnes de briques
    var brickWidth = 60;    // largeur des briques
    var brickHeight = 20; // hauteur des briques
    var brickPadding = 10; // padding entre briques
    var brickOffsetTop = 30;
    var brickOffsetLeft = 30;
    var bricks = []; // creation d'un tableau 2d contenant les colonnes et les lignes de briques[(c),(r)]
...
var lives = 3;
    function mouseMoveHandler(e) { // e.clientX = X de la souris dans la fenetre 
        var relativeX = e.clientX - canvas.offsetLeft;  // e.clientX - dist entre bord gauche du canvas (canvas.offsetLeft)
    ...
    function collisionDetection() { // fonction detection de collision avec une brique
        for(var c=0; c<brickColumnCount; c++) { // pour chaque colonne
            for(var r=0; r<brickRowCount; r++) { // pour chaque ligne
                var b = bricks[c][r]; // stockage de la position x et y de la brique dans une variable b
                if(b.status == 1){
        ...
        for(var c=0; c<brickColumnCount; c++) { // boucle le nombre de colonnes
            for(var r=0; r<brickRowCount; r++) { // boucle nbr de lignes
                    if (bricks[c][r].status == 1){ // si le status de la brique == 1 ;)
                        var brickX = (c*(brickWidth+brickPadding))+brickOffsetLeft; // calcul de la position x de la brique
                        var brickY = (r*(brickHeight+brickPadding))+brickOffsetTop; // calcul de la position y de la brique
